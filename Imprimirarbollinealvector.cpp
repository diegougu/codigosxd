...
private:
    ...
    void Filas(vector<int>& valores, CBinNode* ptr, CBinNode* ptr2);
...
void CBinTree::Filas(vector<int>& valores, CBinNode* ptr, CBinNode* ptr2) {
    if (!ptr && !ptr2) return;

    if (ptr == m_root && ptr2 == m_root) {
        valores.push_back(ptr->value);
        Filas(valores, ptr->nodes[0], ptr2->nodes[1]);
        ptr = ptr->nodes[0];
        ptr2 = ptr2->nodes[1];
    }
        ...
        Filas(valores, ptr->nodes[0], ptr->nodes[1]);
    }
    else if (ptr != nullptr && ptr2 == nullptr) {
        ptr2 = ptr;
        valores.push_back(ptr->value);
        ...
        Filas(valores, ptr->nodes[0], ptr->nodes[1]);
        Filas(valores, ptr2->nodes[0], ptr2->nodes[1]);
    }
